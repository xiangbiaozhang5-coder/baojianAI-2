...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, onSave, showToast }) => {
  ...
  const handleSave = () => {
    ...
    if (!finalBaseUrl) {
        finalBaseUrl = 'https://bj.nfai.lol';
    }
  ...
  const handleTestKey = async (key: string, index: number) => {
    ...
    const cleanUrl = localSettings.baseUrl?.trim().replace(/\/+$/, '') || 'https://bj.nfai.lol';
            ...
            <div>
                ...
                <input 
                    ...
                    onBlur={() => {
                        ...
                        if (!url) url = 'https://bj.nfai.lol'; // Restore default if empty
                        setLocalSettings(prev => ({...prev, baseUrl: url}));
                    }}
                    placeholder="https://bj.nfai.lol"
                ...
                <p className="text-[10px] text-gray-400 mt-1">
                    系统默认使用专用加速接口：<b>https://bj.nfai.lol</b>。<br/>
                    请使用在该平台生成的 <b>API Key</b> (以 sk- 开头)。
                </p>
            </div>
            
            <div className="space-y-3">
                 <div className="flex justify-between items-end">
                     <label className="block text-sm font-medium text-gray-700">API Key 管理 (支持多 Key 轮询)</label>
                ...
                {isBatchEdit ? (
                    <div className="animate-in fade-in zoom-in-95">
                        <textarea 
                            ...
                            placeholder={`sk-ErB5t9tqlMBu0UYptL6UT9E2ljKrBr5hvr5Y0wCUB9vemN1p\nsk-...\nsk-...`}
                            value={batchText}
